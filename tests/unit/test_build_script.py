...
from business_coach.config import ensure_runtime_dirs, get_base_dir
...
class TestGetBaseDirSourceMode:
    ...
    def test_source_mode_returns_project_root(self) -> None:
        ...
        result = get_base_dir()
        # config.py is at src/business_coach/config.py → parents[2] is project root
    ...
    def test_reads_all_expected_keys(self, tmp_path: Path) -> None:
        ...
        toml_path.write_text(
            textwrap.dedent("""\
                [tool.nuitka]
                output-dir = "dist"
                include-package = ["business_coach"]
                include-package-data = ["nicegui"]
...
class TestBuildNuitkaCommand:
    ...
    def test_entry_point_is_always_last(self) -> None:
        """The entry point src/business_coach/main.py is always the last element."""
        config: dict = {
            "output-dir": "dist",
            "include-package": ["nicegui", "dspy"],
            "nofollow-import-to": ["tests"],
        ...
        assert cmd[-1] == "src/business_coach/main.py"
    ...
    def test_data_dir_and_data_files_included(self) -> None:
        ...
        config: dict = {
            "output-dir": "dist",
            "include-data-dir": [
                "src/business_coach/export/templates=business_coach/export/templates",
            ],
            "include-data-files": [
        ...
        assert "--include-data-dir=src/business_coach/export/templates=business_coach/export/templates" in cmd
