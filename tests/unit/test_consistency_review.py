...
class TestConsistencyReviewNode:
    ...
    def test_handles_empty_claims_and_description(self, mock_module_cls):
        ...
        assert result["review_approved"] is False
        mock_instance.assert_called_once_with(claims="", description="", personality_mode="critical", review_notes_text=None)
    ...
    def test_logs_agent_invocation(self, mock_module_cls, caplog):
        ...
        with caplog.at_level(
            logging.INFO, logger="business_coach.agents.consistency_review"
        ):
            consistency_review_node(state)
    ...
    @patch("business_coach.agents.consistency_review.dspy")
    @patch("business_coach.agents.consistency_review.ReviewConsistencyModule")
