...
from business_coach.agents.disclosure import disclosure_node
...
class TestLLMConnectionErrorPropagation:
    ...
    **Validates: Requirements 2.7**
    ...
    def test_disclosure_node_propagates_llm_connection_error(
        ...
        with (
            patch(
                "business_coach.agents.disclosure.InterviewQuestionModule"
            ) as mock_cls,
            patch("business_coach.agents.disclosure.dspy.settings") as mock_settings,
        ):
    ...
    def test_claims_drafting_node_propagates_llm_connection_error(
        ...
        with (
            patch(
                "business_coach.agents.claims_drafting.DraftClaimsModule"
            ) as mock_cls,
            patch(
                "business_coach.agents.claims_drafting.dspy.settings"
            ) as mock_settings,
        ):
            mock_settings.lm = mock_lm
            mock_cls.return_value.side_effect = error
    ...
    def test_consistency_review_node_propagates_llm_connection_error(
        ...
        with (
            patch(
                "business_coach.agents.consistency_review.ReviewConsistencyModule"
            ) as mock_cls,
            patch(
                "business_coach.agents.consistency_review.dspy.settings"
            ) as mock_settings,
        ):
            mock_settings.lm = mock_lm
            mock_cls.return_value.side_effect = error
    ...
    def test_description_drafting_node_propagates_llm_connection_error(
        ...
        with (
            patch(
                "business_coach.agents.description_drafting.DraftDescriptionModule"
            ) as mock_cls,
            patch(
                "business_coach.agents.description_drafting.dspy.settings"
            ) as mock_settings,
        ):
            mock_settings.lm = mock_lm
            mock_cls.return_value.side_effect = error
