...
def get_base_dir() -> Path:
    ...
    When running from source:
        ...
        location (``src/business_coach/`` → project root).
...
def _default_docx_template_dir() -> Path:
    ...
    ``business_coach/export/templates``.  When running from source, they
    live under ``src/business_coach/export/templates``.
    """
    base = get_base_dir()
    if "__compiled__" in dir() or getattr(sys, "frozen", False):
        return base / "business_coach" / "export" / "templates"
    return base / "src" / "business_coach" / "export" / "templates"
